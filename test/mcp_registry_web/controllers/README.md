...
- `sitemap_controller_test.exs` and `feed_controller_test.exs` cover `robots.txt`, the sitemap index and its files (only active listings, a 404 for files that do not exist), and the Atom feed's hub and self links, escaping, and the `<link rel="alternate">` every page carries.
