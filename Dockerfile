...
# Builds for linux/amd64 on Apple Silicon run under emulation, where the BEAM
# JIT's dual-mapped executable memory crashes. Single mapping avoids that and
# costs nothing on native hosts. Affects the build stage only.
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/mcp_registry ./
